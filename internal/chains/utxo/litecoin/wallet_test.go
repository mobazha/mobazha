...
func TestLitecoinWallet_BuildSplitSweepTx_PreservesAffiliateAmount(t *testing.T) {
	w, err := newTestWallet()
	if err != nil {
		t.Fatal(err)
	}
	keyBytes, _ := hex.DecodeString("84c8a01a81bf562aafafd4a9fccda533b33d6382b984c081a8cb7817bf909c18")
	privKey, pubKey := btcec.PrivKeyFromBytes(keyBytes)
	addr, _ := ltcutil.NewAddressWitnessPubKeyHash(ltcutil.Hash160(pubKey.SerializeCompressed()), &chaincfg.TestNet4Params)
	inputs := []iwallet.SweepInput{{TxHash: "bdb237bf8c5de6b60ba1e2dcfe364fc24f583e568d1682f851a9d0f11a45c78d", Value: 1_000_000}}
	rawTx, _, err := w.BuildSplitSweepTx(inputs, *privKey, addr.String(), addr.String(), 100_000, 2)
	if err != nil {
		t.Fatal(err)
	}
	var tx wire.MsgTx
	...
	if _, _, err := w.BuildSplitSweepTx(inputs, *privKey, addr.String(), addr.String(), 1, 2); err == nil || !strings.Contains(err.Error(), "affiliate sweep output is dust") {
