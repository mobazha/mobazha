...
func TestNewRepoWithSharedDB(t *testing.T) {
	...
	if err := r.DB().Update(func(tx database.Tx) error {
		return tx.SetProfile(&models.Profile{Name: "Tenant-isolated public profile"})
	}); err != nil {
	...
	if publicProfile.TenantID != "tenant-A" {
		t.Fatalf("public profile namespace = %q, want SaaS tenant %q", publicProfile.TenantID, "tenant-A")
	}
