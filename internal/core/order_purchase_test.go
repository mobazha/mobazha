...
func TestMobazhaNode_createOrder(t *testing.T) {
	...
	for i, test := range tests {
		order, _, err := network.Nodes()[0].orderService.createOrder(context.Background(), test.purchase)
		if err != nil {
...
func Test_createOrderUnkownVersion(t *testing.T) {
	...
	_, _, err = network.Nodes()[0].orderService.createOrder(context.Background(), purchase)
