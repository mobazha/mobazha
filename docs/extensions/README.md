...
| [Order extension contract](ORDER_EXTENSION_CONTRACT.md) | Generic order-associated resource bindings, reservations, delivery, and conditional settlement |
...
| Bind a domain resource or multi-stage process to an order | Versioned `OrderExtension` contract |
...
`OrderExtension` is not a Collectibles or NFT category. Collectibles is the
first implemented provider. Future limited-inventory, quota, ticket, lot, or
made-to-order providers may use the same lifecycle contract only when they
need its durable binding, reservation, delivery, or attestation semantics;
each keeps its own namespaced type and domain payload.
