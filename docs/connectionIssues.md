...
### Tor
Refer to the guide to [configuring Mobazha to run on Tor](https://github.com/Mobazha/openbazaar-desktop/wiki/Tor-Setup).

More information is available in the [server Tor documentation](https://github.com/Mobazha/openbazaar-go/blob/master/docs/tor.md), if you need it.
...
2. In the config file, ensure Addresses.Gateway has your server's IP embedded in it. For example, if your server ip is `123.45.6.78`, the `Gateway` value would be `/ip4/123.45.6.78/tcp/5102`.
3. In the client, ensure the port set in the server configuration UI matches the port value you set above:

![](https://github.com/Mobazha/openbazaar-desktop/blob/master/imgs/connectionIssues/setPortInUi.png)
...
- In the remote server, in your Mobazha server directory, you can enter the command `go run openbazaard.go setapicreds` and follow the instructions.
...
![](https://github.com/Mobazha/openbazaar-desktop/blob/master/imgs/connectionIssues/sslProtocolError.png)
...
To enable SSL on your remote server, follow this [doc](https://github.com/Mobazha/openbazaar-go/blob/master/docs/ssl.md).

You may also find this [guide to remote server security helpful.](https://github.com/Mobazha/openbazaar-go/blob/master/docs/security.md#basic-authentication)

When you install the Mobazha.crt file on your client machine, be sure to enable it as a trusted certificate.

On OSX, this is done via the Always Trust button.

![](https://github.com/Mobazha/openbazaar-desktop/blob/master/imgs/connectionIssues/osxTrustCertificate.png)
...
![](https://github.com/Mobazha/openbazaar-desktop/blob/master/imgs/connectionIssues/windowsCertManager.png)
...
![](https://github.com/Mobazha/openbazaar-desktop/blob/master/imgs/connectionIssues/sslBadHandshake.png)

It is likely one of two things:

1. You may have not properly specified your server ip when generating the server.csr file. When following this [doc](https://github.com/Mobazha/openbazaar-go/blob/master/docs/ssl.md), be sure to replace \<server-ip\> with the ip of your remote server when executing this command `openssl req -new -key server.key -out server.csr `...
