...
do_install() {
    ...
    echo "  ${BINARY_NAME} start     # Start the node (API on http://localhost:4002)"
    echo "  ${BINARY_NAME} doctor    # Check system health"
    echo "  ${BINARY_NAME} backup    # Back up data"
    echo ""
    echo "After starting, your API endpoint is:"
    echo "  http://localhost:4002"
    echo ""
    echo "Note: The native binary provides the API server only."
    echo "For the full Web UI, use the Docker image:"
    echo "  https://mobazha.org/self-host"
    echo ""
