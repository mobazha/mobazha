...
func (kc *Keychain) KeyForAddress(dbtx database.Tx, addr iwallet.Address, accountPrivKey *hd.ExtendedKey) (*hd.ExtendedKey, error) {
	...
	if record.Change {
		...
		key, err = internalPrivkey.Derive(uint32(record.KeyIndex))
	} else {
		if externalPrivkey == nil {
			return nil, ErrEncryptedKeychain
		}
		key, err = externalPrivkey.Derive(uint32(record.KeyIndex))
	}
	return key, err
}
...
func (kc *Keychain) createNewKeys(dbtx database.Tx, change bool, numKeys int) error {
	...
	for generatedKeys < numKeys {
		...
		if change {
			newKey, err = kc.internalPubkey.Derive(uint32(nextIndex))
		} else {
			newKey, err = kc.externalPubkey.Derive(uint32(nextIndex))
		}
		if err != nil {
			nextIndex++
			continue
