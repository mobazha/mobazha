...
function loadData (options = {}) {
  ...
  this.listenTo(this.moderatorsStatus, 'browseMore', () => onBrowseMore());

  // listen to the websocket for moderator data
  this.serverSocket = getSocket();
}

function clickShowUnverified () {
  togVerifiedShown(false);
  this.trigger('clickShowUnverified');
}

function onBrowseMore () {
  getModeratorsByID();
}

function removeNotFetched (ID) {
  this.unfetchedMods = this.unfetchedMods.filter((peerID) => peerID !== ID);
  checkNotFetched();
}

function processMod (data) {
...
function getModeratorsByID (opts) {
  const op = {
    ...this.options,
    ...opts,
  };
  ...
  if (IDs.length || op.method === 'GET') {
    ...
    const fetch = $.ajax({
      ...
      .done((data) => {
        if (!op.async) {
          data.forEach((mod) => {
            if (!excluded.includes(mod.peerID)) processMod(mod.profile);
          });
          this.unfetchedMods = [];
          checkNotFetched();
...
function removeModeratorsByID (IDs) {
  if (!IDs) {
...
function addMod (model) {
  ...
  this.listenTo(modCard, 'modSelectChange', (data) => {
    if (data.selected) {
      ...
      if (this.options.singleSelect) deselectOthers(data.guid);
...
function modShouldRender (model) {
...
function batchCardRender (model) {
  if (!model || !(model instanceof Moderator)) {
    throw new Error('Please provide a valid profile model.');
  }

  // Render in batches only if the card being added should be visible.
  if (!this.renderTimer && modShouldRender(model)) {
    this.renderTimer = setTimeout(() => { render(); }, 300);
  }
}
...
function deselectOthers (peerID = '') {
...
function togVerifiedShown (bool) {
...
function render () {
  ...
  const showMods = this.modCards.filter((mod) => modShouldRender(mod.model));
