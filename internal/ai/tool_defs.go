...
func SellerTools() []ToolDefinition {
	return []ToolDefinition{
		...
		},
		{
			...
			Description: "Start a product.import run from explicit product import source materials and create reviewable source, candidate, proposal, and validation artifacts. Use only after the user asks to import or organize materials as product listings; this does not directly publish listings. For files attached to the current chat turn, pass the attachmentId/sourceName metadata only; the server injects the attachment payload. After ingest succeeds, call agent_product_import_advance at most once, then summarize the returned state or next action instead of repeatedly listing artifacts.",
			Parameters:  mustJSON(`{"type":"object","properties":{"threadId":{"type":"string","description":"Current agent thread/session ID when known"},"storeId":{"type":"string","description":"Current store/profile ID when known"},"sources":{"type":"array","description":"Product import source materials to stage for review. For current-turn attachments, include attachmentId or sourceName and omit large base64 payloads; the server will attach the file content.","items":{"type":"object","properties":{"attachmentId":{"type":"string","description":"Current-turn attachment ID from chat context, when importing an attached file"},"sourceName":{"type":"string","description":"Filename or human label for the source"},"contentType":{"type":"string","description":"MIME type or logical content type such as text/csv, image/jpeg, or application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"},"text":{"type":"string","description":"Plain text source content for pasted CSV, notes, or supplier data. Do not invent this for binary attachments."},"contentBase64":{"type":"string","description":"Base64-encoded binary source content only when explicitly available in tool arguments; do not reproduce chat attachment base64 manually."}},"required":["sourceName"]}}},"required":["sources"]}`),
		},
		{
