...
func TestMobazhaNode_DeclineOrder(t *testing.T) {
	...
	// Moderated order that is funded — GetUTXOPaymentInfo with moderator creates
	// 2-of-3 escrow so no auto-confirm happens (unlike CANCELABLE).
	orderSub0, err = network.Nodes()[0].eventBus.Subscribe(&events.NewOrder{})
	if err != nil {
		t.Fatal(err)
	}
	...
	paymentData, err := network.Nodes()[1].Wallet().GetUTXOPaymentInfo(
		context.Background(), orderID.String(), network.Nodes()[2].Identity().String(), iwallet.CtMock,
	)
	if err != nil {
		t.Fatal(err)
	}
